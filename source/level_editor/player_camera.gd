...
# The problem is that networks are not merging
func wire() -> void:
	...
	if !has_dragged_wire():
		dragged_wire = wiring_component.create_wire([editor.level.get_path_to(wiring_component), editor.level.get_path_to(wiring_component)])
		return
